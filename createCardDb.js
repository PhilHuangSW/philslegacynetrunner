...
const updateDeck = async () => {
  try {
    const decklist = await fetch('https://netrunnerdb.com/api/2.0/public/decklist/66439');
    const decklistJson = await decklist.json();
    const cards = [];
    for (let ob in decklistJson.data[0].cards) {
      // console.log(`${ob} -- ${decklistJson.data[0].cards[ob]}`);
    ...
    const deck = await pool.query("UPDATE decks SET deck_code = $1, cards = $2 WHERE deck_id = '5' RETURNING *",
      [
        decklistJson.data[0].id,
        cards,
      ],
